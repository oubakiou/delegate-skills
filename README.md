...
> [!WARNING]
> Codex-backed workers run with `codex exec --sandbox danger-full-access`; the child Codex sandbox is not a security boundary. When the requester is also Codex and must launch delegates or the process-contract tests, start it with `codex --sandbox danger-full-access --ask-for-approval on-request` only inside an isolated runner or Dev Container. Anything mounted or authenticated inside that environment—including the repository, Codex/GitHub credentials, and MCP authority—is reachable by the agent. Do not mount the host Docker socket or broad host directories. The included default Dev Container is configured as non-privileged, without Docker-in-Docker or a host Docker socket mount; rebuild it after updating this repository. See the [Dev Container boundary plan](./docs/feature/codex-devcontainer-delegation.md).

Inside the repository Dev Container, start an interactive requester with `scripts/codex-devcontainer.sh [codex arguments...]`. The launcher requires both `DELEGATE_DEVCONTAINER_BOUNDARY=1` and a fixed container runtime marker, then runs Codex with `--sandbox danger-full-access --ask-for-approval on-request`. It fails closed outside that environment and rejects arguments that could override its execution boundary, sandbox, or approval policy, including remote app-server and config/profile overrides. `scripts/codex-devcontainer.sh --unattended [codex exec arguments...]` instead fixes the non-interactive `codex exec` subcommand and bypasses approvals; do not include `exec` yourself. This mode is only for a dedicated run whose repository is trusted and whose credentials, MCP servers, mounts, and egress are independently restricted.

For the VS Code Codex extension, connect to the Dev Container before opening Codex, keep its user-scope `CODEX_HOME` inside the container, and do not bind-mount or copy the host `~/.codex` directory. If the extension needs the same requester mode, set `sandbox_mode = "danger-full-access"` and `approval_policy = "on-request"` in the container-local `~/.codex/config.toml`. Do not add these defaults to the repository `.codex/config.toml`, because project settings also apply when the repository is opened on the host.
