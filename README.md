...
`delegate-prose` covers general-purpose writing, rewriting, and polishing. It fires on length, not on what the task is about: writing 200+ Japanese characters (roughly 100 English words) or a 3+ item bulleted explanation into a document people will read later — Notion, docs, README, an issue body — goes through the skill even when the task is mainly analysis or implementation, while code, structured data, table structure edits, link and number swaps, and one- or two-sentence fixes stay with the main agent. It bundles no style assets: voice, tone, length, and target audience are specified in the request. Facts are taken only from the request and its explicitly named sources, never filled in by guesswork. To finish a piece as an HTML document, pass the generated Markdown to `delegate-htmldoc`. If the user does not specify an output path, generated files go under `delegate-prose-output/`.
...
| `DELEGATE_CHILD_BASH_TIMEOUT_MS`         | `300000` ms (`0` = no injection) | Timeout injected into the **worker's own** Claude Bash tool; **not** the timeout the parent assigns to the delegate invocation |
...
| ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Claude CLI       | `fable`, `opus`, `sonnet`, `haiku`, `claude-fable-5-1`, `claude-fable-5`, `claude-opus-5-5`, `claude-opus-5`, `claude-opus-5[1m]`, `claude-opus-4-8`, `claude-opus-4-8[1m]`, `claude-sonnet-5`, `claude-sonnet-5[1m]`, `claude-sonnet-4-6`, `claude-haiku-4-5`                                                                                                                  | Aliases for Claude family models, plus full `claude-*` model IDs for version pinning (`[1m]` = 1M-context variant)                                                                          |
| Codex CLI        | `gpt-6-astra`, `gpt-6-sol`, `gpt-6-luna`, `gpt-5.6`, `gpt-5.6-sol`, `gpt-5.6-terra`, `gpt-5.6-luna`, `gpt-5`, `gpt-5.5`, `gpt-5.4`, `gpt-5.4-mini`, `gpt-5.4-nano`, `gpt-5.3-codex-spark`                                                                                                                                                                                       | `delegate-imagegen` only accepts the `gpt*` / Codex branch                                                                                                                                  |
| Devin CLI        | `swe-2`, `swe-1.7`, `swe-1.7-lightning`, `swe-1.6`, `swe-1.6-fast`, `devin-glm-5.2`, `devin-deepseek-v4-pro`, `devin-nemotron-3-ultra`, `devin-kimi-k3`, `devin-gemini-3.8-flash`                                                                                                                                                                                               | `devin-*` strips the prefix before passing the model to Devin CLI                                                                                                                           |
| Cursor agent CLI | `composer-2.5`, `composer-2.5-fast`, `cursor-grok-4.5`, `cursor-grok-4.6`, `cursor-grok-4.6-fast`, `cursor-grok-4.7`, `cursor-grok-4.7-fast`, `cursor-gemini-3.1-pro`, `cursor-gemini-3.6-flash-minimal`, `cursor-gemini-3.6-flash-low`, `cursor-gemini-3.6-flash-medium`, `cursor-gemini-3.6-flash-high`, `cursor-kimi-k2.7-code`, `cursor-glm-5.2-high`, `cursor-glm-5.2-max` | `cursor-*` strips the backend selector once, then converts per model (e.g. grok effort → a catalog slug)                                                                                    |
| opencode CLI     | `opencode/opencode-go/glm-5.2`, `opencode/opencode/muse-spark-1.3-contributor-free`                                                                                                                                                                                                                                                                                             | Notation is `opencode/<provider>/<model>` (provider required). Round-trip verified; pick others from `opencode models`. `muse-spark-1.3-contributor-free` trades data for price — see below |
...
opencode's free and contributor models trade data for price. Its documentation describes a contributor tier as heavily discounted token pricing granted in exchange for permission to use your prompts and completions to train future models, and says several free models may use collected data to improve the model; paid models are documented as zero-retention. A delegated run sends repository content to the child process, so treat `opencode/opencode/muse-spark-1.3-contributor-free` and the other zero-cost opencode rows in the price table as unsuitable for code you cannot share. Terms change, so check opencode's own documentation before delegating to one.
...
| `cursor-grok-4.7` / `cursor-grok-4.7-fast` | `low`, `medium`, `high`, `xhigh`                 | Converted to `grok-4.7-<effort>` / `grok-4.7-<effort>-fast` (4.7 catalog slugs carry no `cursor-` prefix) |
...
For Cursor models that use the `cursor-` selector, the selector is the delegate backend selector and must appear exactly once. A doubled prefix (`cursor-cursor-*`) or a direct grok effort catalog slug (`cursor-grok-4.5-low` / `-medium` / `-high`, `cursor-grok-4.6-low` / `-medium` / `-high` / `-xhigh`, `cursor-grok-4.7-low` / `-medium` / `-high` / `-xhigh`, or the corresponding `-fast` forms) stops before dispatch with exit 6; use the `cursor-grok-4.5[@<effort>]`, `cursor-grok-4.6[-fast][@<effort>]`, or `cursor-grok-4.7[-fast][@<effort>]` notation instead. A follow-up that inherits one of these legacy notations from an existing session cannot resume (exit 5); start a new resumable run with the corrected notation.

Without a suffix, delegate-skills does not set an effort override and the target CLI default applies. The documented exceptions are `gpt-5.5`, `gpt-5.4`, and `gpt-5.4-mini` (catalog default `medium`), `cursor-grok-4.5` (catalog default `medium`), `cursor-grok-4.6`, `cursor-grok-4.6-fast`, `cursor-grok-4.7`, and `cursor-grok-4.7-fast` (effort resolves to `high`), and Cursor model slugs that already encode the effort (`-high` / `-max`, and the `cursor-gemini-3.6-flash-*` variants). For Devin, `devin-gemini-3.8-flash` and `swe-2` without a suffix resolve to the catalog default variant `medium`, verified against the CLI; other Devin families were not checked.
...
`cursor-grok-4.7` behaves the same way, except that its catalog slugs carry no `cursor-` prefix: an omitted effort maps to `grok-4.7-high` (non-fast), and `cursor-grok-4.7-fast` maps to `grok-4.7-high-fast` when its effort is omitted.
...
`gpt-6-astra` and `gpt-6-sol` support the same Codex effort suffixes listed above (`low` through `ultra`), as advertised by the local Codex model catalog. `gpt-6-luna` and `gpt-5.6-luna` advertise `low` through `max` only; delegate-skills validates effort per backend, not per model, so an `@ultra` suffix on them passes validation and is left to the Codex CLI to accept or reject. The catalog default is `low` for `gpt-6-astra` and `gpt-5.6-sol`, and `medium` for `gpt-6-sol` and `gpt-6-luna`; without a suffix, the effective effort depends on the target CLI configuration and defaults.
...
When a backend reports measured tokens but no cost (e.g. Codex), the observe usage additionally carries `cost_usd_estimated` converted from this table, kept separate from the measured `cost_usd` field so downstream aggregation can tell the two apart. `cost_estimate_basis` records whether cached-input rates were applied, and the fields are omitted entirely when the table has no usable entry for the model. Backends differ in how they report cached input — Codex and Devin count it inside `input_tokens`, Claude, Cursor and OpenCode report it alongside — so the estimate normalizes to a single prompt total before applying rates; `input_tokens` and `cached_input_tokens` themselves stay as the child CLI reported them. OpenCode carries no estimate at all, because a missing provider cost must stay distinct from a free model's zero. Because `cost_estimate_basis` names the basis of the number, do not aggregate or compare `cost_usd_estimated` across runs whose basis differs.
