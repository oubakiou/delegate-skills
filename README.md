...
> [!WARNING]
> Codex-backed workers run with `codex exec --sandbox danger-full-access`; the child Codex sandbox is not a security boundary. When the requester is also Codex and must launch delegates or the process-contract tests, start it with `codex --sandbox danger-full-access --ask-for-approval on-request` only inside an isolated runner or Dev Container. Anything mounted or authenticated inside that environment—including the repository, Codex/GitHub credentials, and MCP authority—is reachable by the agent. Do not mount the host Docker socket or broad host directories. The included Dev Container currently uses privileged Docker-in-Docker, so Docker Desktop's VM remains the outer boundary; native Linux users should remove that feature or use a separate hardened runner. See the [Dev Container boundary plan](./docs/feature/codex-devcontainer-delegation.md).
