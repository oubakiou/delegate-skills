...
Worker token usage is recorded in observe JSON as `usage.measurement: "measured" | "estimated"` when a run ends. Claude stream-json, Codex JSON/session JSONL, and Devin ATIF export can provide measured values; unsupported or unparsable backends fall back to a chars/4 estimate and emit a `usage_parse_failed` observe event. Estimated usage carries `estimation_basis: "protocol_payload_only"`: it counts only the request/response protocol payload and is a guaranteed lower bound of the worker's real consumption (context reads, tool round-trips, thinking are not included), so do not compare it against measured backends. The cursor backend always falls into this estimate because its CLI reports no usage.
