...
threshold_mode=0
if [ -n "$threshold_pct" ] || [ -n "$min_free_bytes" ]; then
  threshold_mode=1
...
observe_path() {
  ...
  if ! dp="$(df -P -- "$path" 2>/dev/null)" || ! di="$(df -Pi -- "$path" 2>/dev/null)"; then
...
report_server_bin() {
  ...
  local found=0 e g base sz grands
  local sub_found
  for e in "${entries[@]}"; do
    [ -L "$e" ] && continue
    base="${e##*/}"
    if [ -d "$e" ]; then
      ...
      for g in "${grands[@]}"; do
        ...
        if [ "$sz" = "?" ]; then
          printf '  manual-only: %s/%s (size 不明)\n' "$(disp "$base")" "$(disp "${g##*/}")"
        else
  ...
  cat_record "$id" "manual-only" 0 0 0 0 0 0
}
...
for key in "${FS_ORDER[@]}"; do
  repr="${FS_REPR[$key]}"
  if after_df="$(df -P -- "$repr" 2>/dev/null)"; then
...
warn_reasons=""
if root_df="$(df -P -- / 2>/dev/null)"; then
