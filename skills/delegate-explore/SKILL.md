...
- `DELEGATE_EXPLORE_MCP_READ_ONLY=1` で、worker の MCP 利用を読み取り系ツールのみに制限する制約（プロンプトレベル）を注入する。既定は制限なし
...
- read-only。リポジトリのファイル編集・push はしない。MCP は既定では制限なし（`DELEGATE_EXPLORE_MCP_READ_ONLY=1` で読み取り専用制約を注入）
