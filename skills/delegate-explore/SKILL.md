...
- **Claude 系モデル（`fable` / `opus` / `sonnet` / `haiku`）**: WebSearch / WebFetch が技術的に開放される（リポジトリ書き込みツールのみ denylist で除外）。Web 調査を含むリクエストは Claude 系モデルを使う
- **Codex / Devin / Cursor**: WebSearch / WebFetch の可用性は各 CLI の能力・設定次第。確実性が要る Web 調査には使わない
