...
description: >
  ...
  主目的は token cost 削減ではなく capability bridge と context isolation。Codex 限定で起動し、ユーザー向けに画像生成モデル選択の概念は出さない。
...
画像生成・画像編集を Codex 子プロセスへ委譲する。task_type=`imagegen`。既存 delegate のモデル解決や Claude パスは使わず、Codex 限定の capability bridge として扱う。
...
1. **準備**: Objective / Scope / Context / Acceptance criteria / Verification / Constraints の Markdown を stdin で渡す。出力先指定がなければ Constraints に `.temp/imagegen/` を書く。exit 3=前提不足 / exit 4=委譲サイクルなら中止。
   ...
   - `request_file="$(printf '%s' "$out" | jq -r .request_file)"` / `response_file="$(printf '%s' "$out" | jq -r .response_file)"`
2. **Codex 子プロセス起動**:
   - `bash .claude/skills/delegate-imagegen/scripts/delegate-imagegen-codex.sh "$request_file" "$response_file"`
...
- Codex 限定で起動する。Claude パスや `DELEGATE_<TYPE>_MODEL` によるモデル分岐は使わない
- ユーザーに画像生成モデル選択を求めない
