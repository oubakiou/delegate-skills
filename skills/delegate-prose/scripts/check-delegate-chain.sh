...
# 実装は TS (shared/src/check-delegate-chain.ts) にあり、本ファイルは delegate-cli への exec shim。
...
if [ -f "$dir/delegate-cli.mjs" ]; then
  exec node "$dir/delegate-cli.mjs" check-delegate-chain "$@"
fi
exec node "$dir/dist/delegate-cli.mjs" check-delegate-chain "$@"
