...
# 実装は TS (shared/src/wrapper-devin.ts) にあり、本ファイルは delegate-cli への exec shim。
...
if [ -f "$dir/delegate-cli.mjs" ]; then
  exec node "$dir/delegate-cli.mjs" wrapper devin "$@"
fi
exec node "$dir/dist/delegate-cli.mjs" wrapper devin "$@"
