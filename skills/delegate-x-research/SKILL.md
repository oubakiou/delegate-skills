...
対話親では体感待ち時間を隠蔽できる。経路は起動スクリプトで異なる。`delegate-x-research-grok.sh` 経由は `prepare.sh` で `response_file` を事前取得済みなので、`delegate-x-research-grok.sh` を background で実行し、`observe_file` の `state.phase` / `heartbeat` を確認して `ended` または `stalled` になった後に `read-response.sh` する。`run-x-research.sh` 経由は `response_file` を事前に取得できないので、`run-x-research.sh` を background で実行した場合は `read-json.sh .run.response_file "$observe_file"` で応答パスを取るか、合流した出力 JSON をそのまま `read-json.sh` で読む。総所要時間（wall time）は変わらない体感改善であり、非対話モードの親では従来どおりフォアグラウンド実行必須。
