...
allowed-tools: Bash(bash .claude/skills/delegate-x-research/scripts/prepare.sh:*), Bash(bash .claude/skills/delegate-x-research/scripts/delegate-x-research-grok.sh:*), Bash(bash .claude/skills/delegate-x-research/scripts/read-response.sh:*), Bash(jq:*), Read
---
...
2. **調査実行**: 現在は `bash .claude/skills/delegate-x-research/scripts/delegate-x-research-grok.sh "$model" "$request_file" "$response_file"` で Grok CLI を起動する。Grok CLI は X / network access とローカルセッション状態を使うため、sandboxed requester では権限付き実行（Codex では `sandbox_permissions=require_escalated`）で起動する。
