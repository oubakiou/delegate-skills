...
2. **実行系分岐**:
   ...
   - `model` が `swe*` または `devin-*`: `bash .claude/skills/delegate-implement/scripts/delegate-devin.sh "$model" implement "$request_file" "$response_file"`
   - `model` が `composer*` または `cursor-*`: `bash .claude/skills/delegate-implement/scripts/delegate-cursor.sh "$model" implement "$request_file" "$response_file"`
