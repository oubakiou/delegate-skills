...
# 実装は TS (shared/src/read-request.ts) にあり、本ファイルは delegate-cli への exec shim。
...
if [ -f "$dir/delegate-cli.mjs" ]; then
  exec node "$dir/delegate-cli.mjs" read-request "$@"
fi
exec node "$dir/dist/delegate-cli.mjs" read-request "$@"
