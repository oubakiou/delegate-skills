#!/bin/sh
# 正本: shared/delegate-opencode.sh
# 実装は TS (shared/src/wrapper-opencode.ts) にあり、本ファイルは delegate-cli への exec shim。
...
if [ -f "$dir/delegate-cli.mjs" ]; then
  exec node "$dir/delegate-cli.mjs" wrapper opencode "$@"
fi
exec node "$dir/dist/delegate-cli.mjs" wrapper opencode "$@"
