...
| [MUST] Claude backend の stall 検知を実効化する                 | `claude -p` text 出力は完了まで stdout が動かず、正常作業中でも stream idle と見なされる | `--output-format stream-json --verbose` で stdout capture が作業中に動く。短い stall timeout（40 秒）+ timeout 超の実作業 run で誤 kill なしを実測確認 | 完了 |
...
- 実 CLI での手動確認完了後、本ファイルを `docs/archive/delegate-worker-backend-usage-streams.archive.md` に移す（手動確認は 2026-07-18 に全項目完了。リネームのみ残）
...
- [x] Claude backend で `DELEGATE_OBSERVE_STALL_TIMEOUT_SECONDS` を短めに設定し、正常作業中に誤 stall しないことを確認する（2026-07-18: 40 秒設定 + 78 秒の explore 実 run で `stall_timeout` event なし・completed・`usage.measurement == "measured"` を確認）
