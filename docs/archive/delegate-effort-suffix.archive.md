...
[![MKDN](https://img.shields.io/badge/MKDN-review-red?style=for-the-badge)](https://mkdn.review/?url=https%3A%2F%2Fraw.githubusercontent.com%2Foubakiou%2Fdelegate-skills%2Frefs%2Fheads%2Fmain%2Fdocs%2Farchive%2Fdelegate-effort-suffix.archive.md)
...
完了後は README / README_ja の Effort behavior 表と `docs/design/spec.md` に永続情報を移し、本ファイルは archive 済み。
...
- **effort ベースの自動 routing / break-even gate**: [delegate-latency-reduction.md](delegate-latency-reduction.archive.md) §1 スコープ外に記載の別計画のまま。本計画は「宣言できる・記録される」までを扱い、「いつどの effort を使うべきか」の判断は扱わない
...
### Step 5: (完了) ドキュメント反映と archive 化

- [x] README / README_ja: Effort handling 節を suffix 記法前提に書き換え（宣言方法・backend 別開通状況・exit 6 fail-closed・observe 記録と measured/not_exposed の適用範囲）。既存の Effort behavior 表は「suffix 未指定時の既定挙動」表として維持
- [x] 各 SKILL.md: generic 5 skill にインライン env による会話由来 override と exit 6 の扱い（stderr の許容値列挙をユーザー提示に使う）を追加。imagegen / x-research は suffix 非対応（exit 6）を明記
- [x] `docs/design/spec.md`: モデル指定子の正規形（suffix 込み）・分解位置（wrapper 冒頭の共有ヘルパ）・`ORIGINAL_MODEL` / `MODEL` の役割分担・exit code 表への 6 追加
- [x] ローカルインストール済み skill（.claude/.agents、git 管理外）を `gh skill install --from-local --force` で再インストール
- [x] issue #16 へ実装結果をコメントしクローズ（2026-07-19）
- [x] Grok `agent` バイナリとの PATH 衝突は [issue #20](https://github.com/oubakiou/delegate-skills/issues/20) で追跡（§1 スコープ外の「別 issue」）
- [x] Cursor 変換表・許容値セットの鮮度管理運用を development.md「モデル追加・価格更新」チェックリストへ追記
- [x] 本ドキュメントを `docs/archive/delegate-effort-suffix.archive.md` へリネーム済み（2026-07-19）
...
- [delegate-latency-reduction.md](delegate-latency-reduction.archive.md) §1 スコープ外: effort routing は別計画（本計画完了後も routing は扱わない）
