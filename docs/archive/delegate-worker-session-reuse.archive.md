...
Archive 済み。永続情報は design docs / README に反映済み。

[![MKDN](https://img.shields.io/badge/MKDN-review-red?style=for-the-badge)](https://mkdn.review/?url=https%3A%2F%2Fraw.githubusercontent.com%2Foubakiou%2Fdelegate-skills%2Frefs%2Fheads%2Fmain%2Fdocs%2Farchive%2Fdelegate-worker-session-reuse.archive.md)
...
永続情報は `docs/design/spec.md` / `docs/design/protocol-v1.md` / `docs/design/development.md` / README / README_ja に移し、本ファイルは archive 済み。

## 1. 対応スコープ

| 要件                                                              | 開始時の状態                                                                                                    | 完了条件                                                                                                           | 最終状態 | 状態 |
| ----------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------ | -------- | ---- |
| [MUST] follow-up 可能な初回 delegate を明示的に起動できる         | 通常 delegate は非永続で、完了後に resume handle を復元できない                                                 | 親が初回 run を `resumable` として起動でき、observe JSON に resume handle と `run_context` を記録できる            | 実装済み | 完了 |
| [MUST] resumable 利用可否は親が初回起動時に判断する               | delegate 種別ごとの固定挙動にすると、不要な session 永続化か、必要な follow-up 機会の取り逃しが起きる           | 種別は利用可否と推奨条件だけを定義し、親が request ごとに通常 run / resumable initial run を明示的に選べる         | 実装済み | 完了 |
| [MUST] follow-up delegate を明示的に起動できる                    | 各 delegate 実行は必ず新規セッション。親が不具合を見つけた場合も、新しい request と新しい子プロセスで再依頼する | 親が前回の `resumable` run を指定し、同じ backend セッションへ追加指示を送れる                                     | 実装済み | 完了 |
| [MUST] 既存の通常 delegate は非永続・隔離のまま維持する           | Claude は `--no-session-persistence`、Codex は isolated `CODEX_HOME` + `--ephemeral` を使う                     | 初回から `resumable` と指定した run だけ persistence / resume を有効化し、通常実行の挙動は変えない                 | 実装済み | 完了 |
| [MUST] resume handle を observe JSON に記録する                   | `responder_session_id` は wrapper 由来の追跡 ID で、CLI の resume handle ではない                               | backend / model / resume id / persistence mode / source を機械的に読める                                           | 実装済み | 完了 |
| [MUST] follow-up 実行も新しい response_file / observe_file を持つ | request/response は 1 実行 1 ペア。前回 response を更新する設計ではない                                         | follow-up でも新しい run_dir を作り、`followup_of` で前回 run と関連付ける                                         | 実装済み | 完了 |
| [MUST] backend ごとの resume 可否を fail-closed にする            | CLI help 上は resume / continue がある backend でも、現行 wrapper はそれを使わない                              | resume handle が無い、または PoC 未完了の backend では通常新規実行に落とすのではなく、明確なエラーにする           | 実装済み | 完了 |
| [SHOULD] Claude / Codex を初期対象にする                          | Claude / Codex は現行 delegate の主要経路。CLI help で非対話 resume の入口が確認できる                          | live PoC 後、Claude / Codex の resumable initial run と follow-up を実装する                                       | 実装済み | 完了 |
| [SHOULD] Devin / Cursor は PoC 結果に基づき support 対象にする    | print mode + resume + response_file 生成の live PoC は完了済み                                                  | wrapper 統合時に引数契約、usage 抽出、failed response 経路を fixture 化し、unsupported 条件を明確化する            | 実装済み | 完了 |
| [SHOULD] stale context のリスクを親が検出できる情報を残す         | worker の会話文脈が常に新規なので、前回文脈の古さを考慮する必要が薄い                                           | follow-up request に親の最新確認結果、対象 diff、前回 response_file を明示し、observe JSON に `run_context` を残す | 実装済み | 完了 |
...
| ------- | ----------------------------- | ------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------- |
| Claude  | `2.1.195 (Claude Code)`       | `claude --help` / `claude --version` / live PoC                                 | lineage scoped `CLAUDE_CONFIG_DIR`（`.credentials.json` 持ち込み）で `--session-id <uuid>` 初回実行 → session file が `<config_dir>/projects/<slug>/<sid>.jsonl` に永続化 → `-p --resume <sid> --output-format stream-json` で同一 session_id のまま文脈復元と `cache_read_input_tokens` の cache hit を確認した。存在しない id の resume は exit 1                                           | wrapper 統合時の response_file 生成、failed response 経路の fixture 化                   |
| Codex   | `codex-cli 0.142.3`           | `codex exec --help` / `codex exec resume --help` / `codex --version` / live PoC | lineage scoped `CODEX_HOME` + `--ephemeral` なしで session id を stdout JSONL の `thread.started`.`thread_id` から抽出できた。`codex exec resume <id>` で同一 thread_id のまま文脈復元を確認した。resume は cwd を復元せず呼び出し元 cwd を使う（rollout JSONL の `turn_context`.`cwd` で機械確認可能）。sandbox は `-c sandbox_mode=...` で等価指定できる。存在しない id の resume は exit 1 | wrapper 統合時の response_file 生成、failed response 経路の fixture 化                   |
...
2026-07-05 の live PoC で、Devin / Cursor の print mode resume と response_file 生成の併用、および Claude / Codex の resumable initial run / follow-up / session id 抽出 / fail-closed（存在しない id で exit 1）を確認済み。Codex の cwd は resume で復元されないため、wrapper が `cd "$REPO_ROOT"` してから `codex exec resume` を呼び、sandbox は `-c sandbox_mode=...` で再指定する。等価性は rollout JSONL の `turn_context`（`cwd` / `sandbox`）を読んで機械的に検証する。
...
| ------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Claude  | resumable initial run では `--no-session-persistence` を外し、wrapper 生成 uuid の `--session-id` と lineage scoped `CLAUDE_CONFIG_DIR`（`.credentials.json` 持ち込み）を使う。follow-up は `-p --resume <id>`。live PoC で session file の work dir 配下隔離・文脈復元・cache hit を確認済み                                                                                                             | stream-json / dangerous permissions / allowedTools / response_file 生成が現行 wrapper と同じ契約を満たす（resume の CLI surface と隔離は live PoC 済み。wrapper 統合時に fixture 化する）                        |
| Codex   | resumable lineage では初回から `--ephemeral` を外し、lineage scoped `CODEX_HOME` に session JSONL を残す。session id は stdout JSONL の `thread.started`.`thread_id` から抽出する。follow-up は wrapper が `cd "$REPO_ROOT"` してから `codex exec resume <id> -c sandbox_mode=... --json`（resume は `-C` / `--sandbox` を受けず cwd を復元しないため）。live PoC で文脈復元と sandbox 等価指定を確認済み | resume 時の実効 cwd / sandbox が初回実行と同等であることを rollout JSONL の `turn_context`（`cwd` / `sandbox`）で機械的に検証し、wrapper fixture に含める。等価性が崩れた場合は unsupported / fail-closed に戻す |
...
### Step 1: (完了) backend resume live PoC

- 公式ドキュメント確認は Claude / Codex / Devin / Cursor すべて完了済み
- Claude は 2026-07-05 の live PoC で確認済み: lineage scoped `CLAUDE_CONFIG_DIR`（`.credentials.json` 持ち込み）+ `--session-id <uuid>` 初回実行で session file が config dir 配下に隔離され、`-p --resume <sid> --output-format stream-json --dangerously-skip-permissions --allowedTools` の follow-up で同一 session_id のまま文脈復元と cache hit（`cache_read_input_tokens`）を確認した。resume_source は wrapper 生成の `--session-id`（`session_id_arg`）
- Codex は 2026-07-05 の live PoC で確認済み: lineage scoped `CODEX_HOME` + `--ephemeral` なしの `codex exec --json` 初回実行から `thread.started`.`thread_id` を抽出し（`resume_source: codex_json`）、`codex exec resume <id> --json` で文脈復元を確認した。resume は cwd を復元しないため wrapper が `cd "$REPO_ROOT"` してから呼び、sandbox は `-c sandbox_mode=...` で再指定する。等価性は rollout JSONL の `turn_context`（`cwd` / `sandbox`）で機械検証する
- Claude / Codex とも、存在しない session id の resume は exit 1 の明確なエラーになることを確認した（fail-closed に利用できる）
...
成果物: 全 backend supported 判定。session id 抽出 source は Claude=`session_id_arg`（wrapper 生成 uuid）、Codex=`codex_json`（`thread.started`）、Devin=`devin_atif_export`、Cursor=`cursor_create_chat`

### Step 2: (完了) observe JSON schema と helper
...
### Step 3: (完了) prepare / dispatch の follow-up mode
...
### Step 4: (完了) Claude / Codex wrapper 対応
...
### Step 5: (完了) skill docs と親エージェント運用の更新
...
### Step 7: (完了) 永続 docs 反映と archive 化
...
- 本ファイルを `docs/archive/delegate-worker-session-reuse.archive.md` に移す
...
- [x] `DELEGATE_WORK_DIR=.temp/delegate/session-reuse-poc` で Claude resumable initial run が response_file、resumable handle、run_context を生成する
- [x] 同じ Claude session へ follow-up し、新しい response_file が生成される（前回会話の文脈復元も確認）
- [x] 通常 Claude run を follow-up 対象にした場合、prepare が exit 5 で止まる（暗黙 fallback しない）
- [x] `DELEGATE_WORK_DIR=.temp/delegate/session-reuse-poc` で Codex resumable initial run が response_file、resumable handle、run_context を生成する（resume_source: codex_json）
- [x] 同じ Codex session へ follow-up し、新しい response_file が生成される（resume_id 継続・文脈復元も確認）
...
- [ ] worker がコミットを作った resumable initial run の後、follow-up が git_head 検証（run 終了後 HEAD との比較）を通過する（ancestor 許容は自動テストで確認済み）
- [x] resume handle 欠落時に follow-up が prepare exit 5 / wrapper failed response で止まる（手動 + fixture test）
- [x] repo root / worktree root mismatch、または ancestor 関係の無い git head で follow-up が exit 5 で止まる（git_head 改変で手動確認、mismatch は fixture test）
- [x] `vp check`
- [x] `vp test`
...
- [delegate-worker-backend-usage-streams.md](../feature/delegate-worker-backend-usage-streams.md)
