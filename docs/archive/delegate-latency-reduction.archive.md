...
[![MKDN](https://img.shields.io/badge/MKDN-review-red?style=for-the-badge)](https://mkdn.review/?url=https%3A%2F%2Fraw.githubusercontent.com%2Foubakiou%2Fdelegate-skills%2Frefs%2Fheads%2Fmain%2Fdocs%2Farchive%2Fdelegate-latency-reduction.archive.md)
...
完了後は `docs/design/spec.md` / `docs/design/protocol-v1.md` / README / README_ja に永続情報を移し、本ファイルは archive 済み。
...
- **task 種別ごとの tool / MCP capability 絞り込み**: `--tools` による tool schema 削減と task 別 MCP 注入の限定は、prefill 削減として有望だが効果が環境の MCP 設定量に依存する。MCP 継承統一（完了済み: [delegate-worker-mcp-config.archive.md](delegate-worker-mcp-config.archive.md)）の後続計画として切り出し、本計画の Step 1 テレメトリで効果測定の材料だけ先に揃える
- **latency-first の break-even gate / effort・fast 系モデルへの routing**: opt-in の `@effort` suffix 指定は実装済み（[delegate-effort-suffix.archive.md](delegate-effort-suffix.archive.md)）。本計画で扱わないのは、latency を基準に委譲可否や effort・fast 系モデルを機械的に選ぶ gate / routing の自動化で、これは別計画とする。委譲すべきでないタスクの判断は従来どおり各 SKILL.md のコストゲートに委ねる
...
- 本ドキュメントを `docs/archive/delegate-latency-reduction.archive.md` へリネーム済み（2026-07-19）
...
- [ ] Step 1 完了時に着手前ベースライン（backend / model 別。p95 は 20 run 以上）を記録済み（fixtures ベースラインと trivial 単発実測は記録済み。実運用 20 run の蓄積のみ継続中）
...
- [x] Step 5 完了時に §2 と同一の trivial リクエストで削減効果を再計測し、見込み（§5-f）と比較（§4 Step 6 の実測表。worker wall 37s→14〜16s は見込みの目標帯に一致）
- [x] managed-policy 環境相当（bypass 無効）で Claude backend worker の報告回収が成立すること（2026-07-19 検証: `--dangerously-skip-permissions` なしの default 権限モード + `--allowedTools Read` のみで構造化回収が成立、permission_denials 0）
- [x] `npm run sync-shared:check` / `vp check` / `vp test` が全パス（194 テスト、各 Step のコミット時に pre-commit hook でも検証）
...
- 計画テンプレート: [feature-plan-template.md](../feature/feature-plan-template.md)（「公開仕様は遅延させない」原則を §4 の Step 構成に適用）
- 関連計画: [delegate-worker-mcp-config.archive.md](delegate-worker-mcp-config.archive.md)（MCP 継承統一、完了済み。capability 絞り込みはその後続計画として切り出す）
