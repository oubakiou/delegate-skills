...
| --------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | ------ |
| [MUST] observe JSON に実効 effort を記録（issue #16 最小案）    | `run` に effort 相当フィールドなし。実効値は run artifacts を掘らないと分からない                         | `run.effort` が **全 wrapper**（generic 4 backend + imagegen + x-research）で記録される。宣言値（`requested`）と実効値（`effective.value` / `effective.source`）は別フィールドで判別でき、Cursor は `fast` パラメータも記録する                                                              | -        | 未着手 |
| [MUST] `model@effort` suffix による effort 宣言                 | effort の指定経路が存在しない                                                                             | `DELEGATE_<TYPE>_MODEL`（設定 env およびインライン env による会話由来 override。§3.2）で `<model>@<effort>` が使え、Claude / Codex backend の CLI に effort が渡る。Cursor は PoC で変換方式を確定してから開通（確定まで fail-closed）                                                       | -        | 未着手 |
| [MUST] 非対応 backend・不正値の fail-closed                     | -                                                                                                         | Devin backend への `@` 指定、backend 許容外の effort 値、Cursor の未確定変換・slug（`-high` / `-max`）二重指定は dispatch 前に明示エラー（exit 6）で停止し、黙って無視しない                                                                                                                 | -        | 未着手 |
| [MUST] resumable / follow-up での suffix 保持                   | Claude / Codex wrapper は単一の `$MODEL` 変数で observe の `usage.model` / `backend_session.model` も書く | 初回 resumable run で `backend_session.model` に suffix 込み文字列が記録され、follow-up が前回指定子（suffix 込み）を継承して同じ effort フラグで再起動されることが e2e テストで検証されている。dispatch / wrapper 直接起動で異なる指定子を渡した場合は既存 validation が fail-closed にする | -        | 未着手 |
| [SHOULD] suffix 付きモデル名の下流互換（価格 lookup / metrics） | 価格 lookup は `gpt-5.5@high` にマッチせず cost estimate が付かない（実測確認済み）                       | 価格 lookup・`cost_usd_estimated` が suffix を剥離して従来どおり機能する。metrics / observe の model 系フィールドの扱いが §5-b の正規形どおりに統一されている                                                                                                                                | -        | 未着手 |
...
| ------- | ------------------------------------------------------------------------------------------- | --------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Claude  | `--effort <level>`                                                                          | help に列挙なし                               | フラグ実在を確認（claude 2.1.x `--help`）。許容値・alias モデルへの適用可否は Step 2 PoC                                                                                                                                         |
| Codex   | `-c model_reasoning_effort=<value>`                                                         | `low \| medium \| high \| xhigh`（README 表） | `-c` override は followup の `sandbox_mode` で使用実績あり（`shared/delegate-codex.sh:151-156`）。session JSONL への反映は Step 2 PoC                                                                                            |
| Cursor  | 独立フラグ**なし**。`--model 'model[effort=high,fast=false]'` の bracket parameter override | 不明（モデル別）                              | agent 2026.07.16-899851b で `--effort` は unknown option（実測）。help に bracket 記法の案内あり。cli-config.json ではモデルによりパラメータ名が `effort` / `reasoning` に分かれる（レビュー指摘）。変換方式は Step 2 PoC で確定 |
...
| Backend | `@effort` 指定時                                                                                                                           | 許容値検証                       | 未指定時               |
| ------- | ------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------- | ---------------------- |
| Claude  | `--effort <v>` を追加                                                                                                                      | PoC で確定した許容値セット       | フラグなし（現状維持） |
| Codex   | `-c model_reasoning_effort=<v>` を追加                                                                                                     | `low \| medium \| high \| xhigh` | フラグなし（現状維持） |
| Cursor  | **PoC 確定まで fail-closed**。確定後は `--model 'model[<param>=<v>]'` の bracket override（`<param>` はモデル別に `effort` / `reasoning`） | PoC で確定したモデル別セット     | 現状維持               |
| Devin   | エラー（指定手段なし）                                                                                                                     | -                                | 現状維持               |

fail-closed は **prepare 時点**（dispatch 前）で行う。`prepare.sh` の既存 exit code（3=前提不足 / 4=委譲サイクル / 5=follow-up validation 失敗）に **exit 6=effort 指定不正** を追加する。wrapper 側でもヘルパ経由で同じ検証を通し、wrapper 直接起動の経路でも黙って無視されないようにする（二重検証。判定はヘルパ一箇所なので乖離しない）。

Cursor の宣言経路は、独立フラグが存在せず bracket override のパラメータ名がモデル別（`effort` / `reasoning`）である以上、モデル別変換の確定なしに実装できない。PoC（Step 2）でモデル × パラメータ名 × 許容値を確定するまで、`cursor-*` / `composer-*` への `@` 指定は exit 6 で停止する。slug（`-high` / `-max`）と `@` の二重指定は PoC 結果に関わらず常に禁止する。
...
| ----------------- | --------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------- |
| `measured`        | run artifacts から実効値を抽出できた                                                    | Codex（session JSONL `reasoning_effort` 非 null）/ Cursor（slug または cli-config parameters） |
| `backend_default` | CLI 既定に委ねられたことが artifacts で確認できた（例: Codex `reasoning_effort: null`） | Codex                                                                                          |
| `not_exposed`     | backend が実効値を露出していない（`effective.value: null`）                             | Claude / Devin / Grok（x-research）                                                            |
...
### Step 1: (未着手) 実効 effort / fast の記録（issue #16 最小案）
...
- Codex: `delegate_observe_usage_from_codex_sessions` と同じ session JSONL 探索で `reasoning_effort` を抽出（非 null → `measured`、`null` → `backend_default`）。`delegate-imagegen-codex.sh` にも同 helper を適用
- Cursor: model slug の `-(high|max)` 終端 → `measured`。隔離 cli-config.json の selectedModel parameters（`effort` / `reasoning` / `fast`）からの抽出を実 config で確認し、取れる範囲で `measured`、取れなければ `not_exposed` に倒す。`fast` は `effective.fast` に記録
- Claude / Devin / Grok（x-research）: `effective: {value: null, source: "not_exposed"}` を明示記録
- `scripts/observe-json.test.ts` に JSONL fixture / null fixture / cli-config fixture（effort・reasoning・fast）/ not_exposed の shell test を追加

成果物: 宣言経路なしでも「この run が何 effort / fast で走ったか」が全 wrapper の observe JSON から機械可読になる（issue #16 の要求範囲を充足）

### Step 2: (未着手) PoC — CLI ごとの effort 受理確認

実モデルを呼ぶ live 実行で、**マッピング設計の確定に必要な事実**を先に固める:

- Claude: `--effort` の許容値セットと、alias モデル（`haiku` 等）への適用可否
- Codex: `-c model_reasoning_effort=high` が session JSONL の `reasoning_effort` に反映されること（= requested / effective 突合の土台）
- Cursor: bracket parameter override（`--model 'model[effort=high]'` 等）の受理可否、モデル別のパラメータ名（`effort` / `reasoning`）と許容値、slug との併存挙動。ドキュメント済み Cursor モデル（`composer-2.5` 系 / `cursor-*` 系）ごとに確認する
- 確定結果を §3.3 の表・検証ヘルパの許容値セット・§5-c の採否へ反映する。Cursor で一様な変換が成立しないモデルは fail-closed 対象として README に明記する

成果物: backend 別マッピングの「PoC」項目解消。Cursor の宣言経路を開通できるか（どのモデルで開通するか）の確定

### Step 3: (未着手) 分解ヘルパと prepare 検証

- `delegate_observe_split_model_effort` を `shared/observe-json.sh` に追加（`@` 分解 + PoC 確定済み許容値による backend 別検証）
- `shared/prepare.sh` に検証を組み込み、exit 6（effort 指定不正）を追加。Devin backend への `@`、許容外の値、Cursor の未確定モデル・slug 二重指定をここで停止。stderr メッセージは §3.2 の契約に従う
- README 記載の全ドキュメント済みモデル名 × suffix 有無で分解が正しいことを in-source test で検証

成果物: 不正な effort 指定が子プロセス起動前に明示エラーで止まる

### Step 4: (未着手) wrapper 改修と下流互換

- Claude / Codex wrapper に `ORIGINAL_MODEL` / `MODEL` 分離を導入し、observe 書き込み（`run.model` / `usage.model` / `backend_session.model`）を `ORIGINAL_MODEL` に統一。`delegate-claude.sh` / `delegate-codex.sh` は effort があれば §3.3 のフラグを argv に追加。`delegate-cursor.sh` は PoC で開通が確定したモデルのみ bracket override を適用。`delegate-devin.sh` はヘルパ検証のみ（fail-closed）
- 価格 lookup（`shared/observe-json.sh:781-808`）の `normalized_model` に `@` 以降の剥離を追加し、`cost_usd_estimated` が suffix 付きでも従来どおり付くことをテスト
- `run.effort.requested` の記録と `effort_mismatch` event（requested / effective 突合）を wrapper 終端に組み込み
- `scripts/delegate-wrapper-session.test.ts` に §6 の argv assert・fail-closed・resumable → follow-up suffix 保持 e2e を追加
...
- [ ] Step 2 PoC の全項目を実 CLI で確定済み（Claude 許容値 / Codex JSONL 反映 / Cursor bracket override のモデル別挙動）
- [ ] `DELEGATE_EXPLORE_MODEL=gpt-5.5@high` の実 delegate で observe JSON に `run.effort: {requested: "high", effective: {value: "high", source: "measured"}}` と `cost_usd_estimated` が両方付くこと
- [ ] `@` なしの実 delegate で、起動 argv が本実装前とバイト単位で同一であり、observe JSON の差分が `run.effort`（`requested: null` + `effective`）の追加のみであること
...
| 宣言したのに CLI に効いていない（フラグ無視・バージョン差）                         | requested / effective の突合で検出し `effort_mismatch` event を追記する。実効値を露出しない backend（Claude / Devin）は README の Effort behavior 表に検証不能である旨を明記 |
