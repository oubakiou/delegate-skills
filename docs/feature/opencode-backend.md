...
| --------------------------------------------- | ------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------- | ---------------------- |
| [MUST] opencode を 5 番目の実行系へ分岐       | 未           | `backendFromModel('opencode/...')` が `opencode` を返し、dispatch が `delegate-opencode.sh` を起動する in-source test が通る                                                                                                                             | `backendFromModel` が `opencode` を返し、dispatch が `delegate-opencode.sh` を起動する。未登録 backend は exit 2            | 完了                   |
| [MUST] 通常 run の protocol v1 往復           | 未           | explore / implement / chore / review / htmldoc の 5 種別で fake CLI golden が通り、実 CLI で 2 種別以上が `completed` を返し、その Summary が request の対象を指している（テンプレート文字列の反復でない）ことを目視確認する                             | 5 種別の fake CLI golden が通り、実 CLI の explore / implement が `completed`。Summary は request を反映                    | 完了                   |
| [MUST] read-only 種別の direct edit-tool 抑止 | 未           | explore / review で worker が `edit` / `write` ツールを呼べないことを実機で確認する（Claude パスの denylist と同等の範囲。bash 経由・task 経由の書き込みは抑止対象外）                                                                                   | 採用 profile で `write` が unavailable tool になり、実 CLI smoke でも対象ファイルが作られない。bash / task 経由は抑止対象外 | 完了                   |
| [MUST] measured usage / cost の記録           | 未           | observe JSON の `usage.measurement` が `measured`、`source` が `opencode_step_finish`、token 内訳と `cost_usd` が `step_finish` 由来                                                                                                                     | `usage.measurement = measured` / `source = opencode_step_finish`、実 CLI で cost 0.02685684 を記録                          | 完了                   |
| [MUST] 不正なモデル記法の fail-closed         | 未           | provider 省略形と二重 selector が dispatch 前に exit 6 で停止し、stderr に許容形式を列挙する                                                                                                                                                             | provider 欠落・selector 欠落・区切り誤り・空白混入が exit 6。未知 provider は通す                                           | 完了                   |
| [MUST] 生成物の同期と配布                     | 未           | `npm run build` → `sync-shared` の後、`build:check` / `sync-shared:check` / `check-no-jq-md2idx` / `vp check` / `npm test` が通る                                                                                                                        | `build:check` / `sync-shared:check` / `check-no-jq-md2idx` / `vp check` / `npm test`（737）が通る                           | 完了                   |
| [MUST] 補助 subprocess の頑健性               | 未           | `export` / `models --verbose` が応答しない場合でも run 全体が hang せず、timeout・出力上限・SIGKILL で打ち切られる。`--version` の失敗は exit 3 で停止し、`export` / `models --verbose` の失敗は telemetry 欠落として run を止めない                     | `--version` は exit 3 の preflight、`export` / `models --verbose` / `session delete` は bounded helper 経由の fail-soft     | 完了                   |
| [MUST] 失敗の検知と通知                       | 未           | catalog に無いモデルでの失敗が `error.kind = "model_catalog_miss"` に記録され、failed response の Error section 経由で main へ届く（effort 警告は effort を実装する場合の条件として SHOULD 側に置く）                                                    | 実 CLI で `error.kind = model_catalog_miss` / `retryable: true` を記録し、Error section 経由で main へ届く                  | 完了                   |
| [MUST] session lifecycle                      | 未           | session ID を取得できた通常 run が session store を残さず、取得できなかった run は `session_delete_skipped`、削除失敗は `session_delete_failed`、削除の timeout は `session_delete_failed` として必ず observe に残る（残留を許すのは記録された場合だけ） | 通常 run 後に `session list` が増えない。ID 取得不能は `session_delete_skipped`、削除失敗は `session_delete_failed`         | 完了                   |
| [SHOULD] session reuse                        | 未           | resumable で `sessionID` を回収し、follow-up が `-s <sessionID>` で継続する golden が通る                                                                                                                                                                | 実 CLI の resumable → follow-up で、request にパスを書かずに直前の文脈からファイルを特定して更新した                        | 完了                   |
| [SHOULD] MCP 注入                             | 未           | `DELEGATE_OPENCODE_MCP_SOURCE` 指定時に親 MCP 設定を config の `mcp` セクションへ変換注入し observe に server 名のみ記録する。未指定時は `source: "shared"` を記録する                                                                                   | 決定表 4 行を golden で固定。注入値は capture / observe へ出る前にマスクされる。**実効性は本環境で未検証**（§8）            | 完了（実効性は未検証） |
| [SHOULD] effort（`--variant`）対応            | 未           | `@<effort>` が形式検証のみで `--variant` へ渡り、`run.effort.requested` / `effective` が記録される。**実装する場合は**未対応値の `effort_unsupported` event と Summary 警告行の到達まで含めて完了とする                                                  | 形式検証のみで素通し。実 CLI で `effort_unsupported` と Summary 先頭の警告を確認                                            | 完了                   |
| [SHOULD] 価格表エントリ                       | 未           | `shared/model-token-prices.json` に opencode の pricing source と主要モデルを追加し、チャートを再生成する                                                                                                                                                | `opencode models --verbose` の `cost` から 29 モデルと pricing source を追加し、チャート 2 枚を再生成                       | 完了                   |
...
| 最終応答の形式                              | `opencode/nemotron-3.5-lightning-free` の run で、回答内容は正確なまま本文が先に置かれ front-matter が末尾に来て failed になった。同じ request を `opencode-go/glm-5.2` で回すと形式を守った。ただし**これはモデルの価格帯による差ではなく出力形式の指示追従の問題**で、高 effort の run では他モデルでも起き得る（[#35](https://github.com/oubakiou/delegate-skills/issues/35)） |
...
front-matter の出力はプロンプト依存になるため、front-matter を含まない応答は failed response として扱い、wrapper が status を落とす。この依存は実測で顕在化しており（#35）、モデルの価格帯や能力ではなく出力形式の指示追従に起因する。高 effort の run ほど崩れやすいため、モデル選定では回避できない前提で扱う。
...
| 最終応答の front-matter が欠落・位置崩れして failed になる                                              | **実測で発生**（2026-08-23、[#35](https://github.com/oubakiou/delegate-skills/issues/35)）。回答内容は正確なまま本文が先に置かれ、front-matter が末尾（closing delimiter なし）に来た。これは**出力形式の指示追従の問題でモデルの価格帯や能力とは独立**しており、高 effort で推論が長い run ほど起きやすい傾向がある。worker prompt は「先頭に front-matter を置き」と明示しており指示は曖昧ではない。現状は failed のまま扱い、Error section で front-matter 欠落と分かるようにする。位置に寛容な parse への緩和は本文中の `status:` を誤検出するリスクと引き換えになるため、#35 で別途判断する |
