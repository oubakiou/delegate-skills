...
| [SHOULD] 子 CLI が無音でもラッパの生存を判定できる | response_file ができるまで外側から停滞と低速を判別しづらい                                             | 子プロセス実行中に observe JSON の heartbeat を定期更新する                                                                          |
...
jq -r '.heartbeat.ts // empty' "$observe_file"
jq -r '.state.duration_ms // 0' "$observe_file"
```

高度な状況把握へ進む条件は利用側が決める。例:

- heartbeat が一定時間更新されない
- `state.duration_ms` が timeout 閾値を超える
...
| `state`          | `phase`, `pid`, `started_at`, `ended_at`, `exit_code`, `duration_ms`, `response_present`                  | `dispatch.sh`                |
| `heartbeat`      | `ts`, `pid`, `backend`                                                                                    | wrapper helper               |
...
lock file は observe file basename 派生の `<pair>_observe.lock` とし、`run_dir` 配下に置く。heartbeat background process、dispatch start/end、stream import、failed response event はすべて `observe-json.sh` 経由で同じ lock を使う。

`streams.*.content` は保存上限を持つ。初期値は `DELEGATE_OBSERVE_STREAM_MAX_BYTES`、未設定時は 65536 bytes とする。超過時は末尾を残して `truncated: true` と `bytes` に元の総量を記録する。全量ログが必要な用途は上限を明示的に上げ、`0` は無制限として扱う。
...
```json
{
  ...
  "events": [
    ...
    { "kind": "dispatch_start", "ts": "2026-07-04T12:34:57Z", "backend": "claude", "pid": 12345 },
    { "kind": "response_missing", "ts": "2026-07-04T12:49:21Z" },
    { "kind": "dispatch_end", "ts": "2026-07-04T12:49:21Z", "backend": "claude", "exit_code": 1 }
  ],
  "streams": {
...
```bash
jq '{phase: .state.phase, heartbeat: .heartbeat.ts, exit_code: .state.exit_code}' "$observe_file"
...
### Step 2: (未着手) run_dir 生成と observe helper
...
### Step 3: (未着手) dispatch event と exit code 伝播
...
- `DELEGATE_METRICS_FILE` にも必要最小限の dispatch event を任意で追記する

成果物: dispatch の開始・終了・exit code が観測できる状態

### Step 4: (未着手) CLI stdout / stderr 保全と heartbeat
...
- 子 CLI 実行中だけ observe JSON の heartbeat を定期更新する
...
### Step 6: (未着手) ドキュメント更新と同期

- README / README_ja に `DELEGATE_WORK_DIR=.temp/delegate/work` の推奨例を追加する
- `docs/design/spec.md` / `docs/design/development.md` に観測ファイル規約を追加する
- `npm run sync-shared` で `skills/*/scripts` へ同期する
- `npm run sync-shared:check` / `vp check` / `vp test` を実行する
...
| 候補                                         | 採用 | 理由                                                                    |
| -------------------------------------------- | ---- | ----------------------------------------------------------------------- |
| **observe JSON の `heartbeat` を上書き更新** | ✓    | 子 CLI が無音でも「ラッパは生きている」を判定でき、ファイル数も増えない |
| 独立 heartbeat ファイル                      | ✗    | mtime 監視は単純だが、sidecar ファイルが増える                          |
| CLI stdout の進捗だけを見る                  | ✗    | CLI によって stdout の粒度や有無が異なる                                |
...
- fake CLI による wrapper 検証
  ...
  - 子プロセス実行中に observe JSON の `heartbeat` が更新される
...
- [ ] `vp check`
- [ ] `vp test`
- [ ] `DELEGATE_WORK_DIR=.temp/delegate/work` で delegate 実行し、response_file basename 派生の observe JSON を確認する
- [ ] `jq '{phase: .state.phase, heartbeat: .heartbeat.ts, exit_code: .state.exit_code}' <observe_file>` で監視に必要な部分だけ読める
- [ ] 子 CLI を意図的に失敗させ、failed response と observe JSON の stderr 抜粋を確認する
- [ ] `*_req.md` を開き、人間用の依頼内容確認として読めることを確認する
- [ ] `*_res.md` を開き、人間用の作業報告として読めることを確認する
- [ ] README / README_ja / design docs の公開仕様と実装が一致している
