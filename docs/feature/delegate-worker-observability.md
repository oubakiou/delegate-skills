...
| `heartbeat`      | `ts`, `pid`, `backend`, `stdout_bytes`, `stderr_bytes`, `last_stream_change_at`                           | wrapper helper               |
...
- [ ] `jq '{phase: .state.phase, heartbeat: .heartbeat.ts, stdout_bytes: .heartbeat.stdout_bytes, stderr_bytes: .heartbeat.stderr_bytes, exit_code: .state.exit_code}' <observe_file>` で監視に必要な部分だけ読める
