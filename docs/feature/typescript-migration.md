...
`shared/` 配下のシェルスクリプト実装（約 4,800 行 + 専用 wrapper 2 本）を TypeScript へ移行し、esbuild で単一ファイルにバンドルして配布する。これによりユーザー前提条件から `jq` と `md2idx`（`npx` 実行）を撤廃し、**Node.js（24 以上）と各バックエンド CLI のみ**にする（`.sh` shim の実行に POSIX シェル、follow-up 検証に git は引き続き使う）。あわせてテストを in-source testing へ寄せ、bash snippet を文字列で組み立てる現行 fixture 方式を縮退させる。完了後は spec.md / development.md に永続情報を移し、本ファイルは archive する。
...
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- | -------- | ------ |
| [MUST] `shared/` 全スクリプトの TS 化（resolve-model / check-delegate-chain / build·read-request / build·read-response / prepare / dispatch / run / observe-json / backend wrapper 4 本 / prompt-constraints / delegate-mcp） | 未（全て bash、`shared/*.sh` 17 本）                                              | 各スクリプトの argv / stdout / exit code 契約を変えずに TS 実装へ置換。`.sh` は**直接実行されるエントリポイント**の exec shim のみ残る       |          | 未着手 |
| [MUST] 専用 wrapper / 専用 run の TS 化（`prepare-imagegen` / `run-imagegen` / `delegate-imagegen-codex` / `run-x-research` / `delegate-x-research-grok`）                                                                    | 未（skill 配下の独立正本 bash）                                                   | 共通 TS モジュールを import する形で置換し、コピー由来重複を解消                                                                             |          | 未着手 |
| [MUST] `jq` 依存の撤廃                                                                                                                                                                                                        | runtime で jq 必須（`shared/` 内 135 箇所超 + 全 SKILL.md の手順・allowed-tools） | 配布物の実行パスと**全 SKILL.md の手順・allowed-tools** に jq への参照が存在しない（静的検査で確認）。README の Prerequisites から jq を削除 |          | 未着手 |
| [MUST] `md2idx` の runtime `npx` 撤廃                                                                                                                                                                                         | 全 skill が `npx --yes md2idx` に依存                                             | md2idx を npm 依存として import しバンドルに内包。`check-md2idx.sh` 相当の前提チェックが不要になる                                           |          | 未着手 |
| [MUST] 単一ファイルバンドルの self-contained 配布                                                                                                                                                                             | 未                                                                                | `shared/dist/delegate-cli.mjs` 1 ファイル + shim 群を sync-shared で各 skill へ配布し、`gh skill install` 単体で動作                         |          | 未着手 |
| [MUST] 公開契約の不変                                                                                                                                                                                                         | —                                                                                 | `run.sh` の JSON envelope / observe JSON スキーマ / protocol-v1 の request·response 形式 / 各スクリプトの exit code 表が不変                 |          | 未着手 |
| [SHOULD] テストの in-source 移行                                                                                                                                                                                              | TS テスト 2 本が bash snippet fixture で約 3,900 行                               | pure ロジックは実装隣接の in-source test へ。fake CLI による wrapper 統合テストは golden として維持                                          |          | 未着手 |
| [SHOULD] CI による merge gate                                                                                                                                                                                                 | 未（workflow なし、検証は pre-commit hook と手動）                                | pinned Node / Linux の CI で clean checkout からの build byte 比較・`sync-shared:check`・`vp check` / `vp test` を PR 必須化                 |          | 未着手 |
| [SHOULD] ドキュメント追従（README / README_ja / spec.md / development.md / 全 SKILL.md）                                                                                                                                      | 全て bash + jq 前提の記述                                                         | Prerequisites・アーキテクチャ図・開発手順・skill 手順が TS 実装と一致                                                                        |          | 未着手 |
...
| `md2idx` npm package（v0.2.0、zero-dependency ESM、library exports） | `import` してバンドルに内包。ただし v0.2.0 は engines `>=24.0.0` を宣言し、library と CLI が同一 entry（`dist/md2idx.mjs`）で top-level の CLI 自己判定（`process.argv[1]` と `import.meta.url` の realpath 比較）を持つため、**副作用のない library entry を分離した md2idx 側リリースを Step 1 の前提**にする |
...
shared/src/**/*.ts            正本（TS モジュール + in-source test）
   │  esbuild --bundle --platform=node --format=esm --target=node24
   ▼
...
| `shared/dist/delegate-cli.mjs`  | esbuild 生成の単一ファイル CLI。**git にコミットする**                                                                                                                                                                                                              | 生成物だがコミット対象（`gh skill install` はリポジトリ内容をそのまま配布するため）。lint 対象外（既存 `ignorePatterns: dist/`） |
| `scripts/build-cli.ts`          | esbuild 呼び出し（決定的ビルド設定）                                                                                                                                                                                                                                | 開発ツール                                                                                                                       |
...
- **lock**: 現行 `observe-json.sh` は flock 優先 + mkdir フォールバック。Node に flock が無いため、**移行に先立ち bash 側を mkdir lock に統一**する（Step 2）。ただし現行フォールバックは「`mkdir` 成功まで無期限 sleep、正常終了時のみ `rmdir`」で、owner 記録・timeout・stale 判定・シグナル時 cleanup を**持たない**。統一時にこれらを新規に仕様化しないと、lock 保持中の kill / crash で全 observe 更新が恒久停止する（Step 2 で対応）
...
### Step 1: (未着手) md2idx 前提リリースとビルド・配布基盤

- **md2idx 側の先行リリース**（本移行の前提。md2idx リポジトリで実施）: CLI トリガを bin 専用 entry に分離し、library entry を top-level 副作用ゼロにする。engines 宣言と最小サポート Node の整合もここで確定する
- `esbuild` を devDependency に追加、`scripts/build-cli.ts` と `npm run build` / `npm run build:check`（再ビルド byte 比較）を作成
...
### Step 2: (未着手) observe lock の mkdir 統一と stale lock 仕様化

- `shared/observe-json.sh` の flock 優先分岐を削除し mkdir lock に統一する
- 現行フォールバックに存在しない stale lock 対策を**新規に仕様化**する: lock dir 内への owner 情報（PID + token）記録、bounded wait（timeout 超過時のエラー化）、mtime / lease による stale 判定と安全な takeover、trap による異常終了時 cleanup
- テストを `scripts/observe-json.test.ts` に追加: 並列 dispatch を模した競合 / **lock 保持者の kill 後に後続が回復できる** / 残存 lock dir の takeover / bounded wait の timeout

成果物: bash / TS が相互排他可能で、保持者異常終了から回復できる単一 lock プロトコル

### Step 3: (未着手) 変換・検証系 leaf の TS 化（md2idx バンドルの中核）

- `resolve-model` / `check-delegate-chain` を TS 化（3a。いずれも直接実行される leaf）
- `build-request` / `read-request` / `build-response` / `read-response` を TS 化し、md2idx を import に置換（3b。3a / 3b は別 PR 可）
...
### Step 4: (未着手) observe モジュールの TS 化（bash 版は凍結並存）

- 4a: pure 系（usage 抽出 / effort 検証 / cost 推定 / timing 計算）を TS モジュール化し、`observe-json.test.ts` の対応 describe を in-source test へ移行
- 4b: mutate 系（lock·init / session / lifecycle / response assembly / stream 取り込み）を TS モジュール化する。この時点の consumer はまだ bash（prepare / dispatch / wrapper）なので、bash 版 `observe-json.sh` は**凍結**（機能追加禁止）して並存させ、スキーマ golden と統一 lock で等価性を担保する
...
### Step 5: (未着手) prepare / dispatch / run / 専用 run / metrics の TS 化

- `prepare`（session_mode 解析・follow-up 検証含む）/ `dispatch` / `run`（one-shot envelope）を TS 化し、observe は Step 4 の TS モジュールを in-process 利用
- `run-imagegen.sh` / `run-x-research.sh` は `run.sh` を source して関数 override する構造のため、**共通 run と同一 Step で** TS 化する（`prepare-imagegen` を含む）
- `DELEGATE_METRICS_FILE` 追記を共通モジュール化し、`fixtures/metrics/` baseline の一致を確認
- 完了時に反復ベンチ（§6）で移行前とのレイテンシ比較を行う

成果物: 親側 happy path（run 一発）と専用 run が wrapper 起動まで TS で完結

### Step 6: (未着手) backend wrapper クラスタの TS 化

- `delegate-claude` / `delegate-codex` / `delegate-cursor` / `delegate-devin` を TS 化（backend ごとに PR 分割可）
- wrapper が source する `prompt-constraints.sh` / `delegate-mcp.sh` も**同一クラスタとして** TS 化する（`scripts/delegate-mcp.test.ts` を golden に使う）
- 共通部（引数解析 / finish_without_child / prompt 骨格 / wait・cleanup / response 補完 / usage・effort 記録）は型付き設定オブジェクトを受ける共通モジュールにし、backend 固有部（Claude の schema / allowedTools、Codex の HOME 隔離・E2BIG gate、Cursor の bracket 変換、Devin の prefix・ATIF）のみ各実装に残す
- 子プロセス管理（spawn / stdin / タイムアウト / stall 検知 / シグナル・cleanup）の等価性を fake CLI テストで検証
- 完了時に反復ベンチ（§6）で移行前とのレイテンシ比較を行う

成果物: 4 backend の dispatch 実行系と wrapper 用ライブラリが TS 化され、横断変更が 1 モジュールで完結

### Step 7: (未着手) 専用 wrapper（imagegen / x-research）の TS 化

- `delegate-imagegen-codex` / `delegate-x-research-grok` を、Step 6 の共通モジュール + Codex 起動モジュールを import する形で TS 化
- dispatch を経由しない observe 記録の二重化がないことを確認
- ここで bash consumer が全滅するため、凍結していた bash 版 `observe-json.sh` を削除できる状態になる（削除自体は Step 8）

成果物: 独立正本だった専用 wrapper のコピー由来重複が解消し、全実行経路が TS 実装になる

### Step 8: (未着手) 後始末と公開仕様更新

- shim 以外の .sh 実装本体・`check-md2idx.sh`・凍結版 `observe-json.sh`・bash snippet fixture 残骸を削除
- **全 7 skill の SKILL.md を更新**: 手順中の envelope / observe JSON の読み取りを jq 非依存へ置き換え（CLI の読み取り用サブコマンド化、または値の直接出力）、allowed-tools から `Bash(jq:*)` を撤去
- **配布 tree の静的検査**を追加: `skills/` / `.claude/skills/` / `.agents/skills/` 配下に `jq` / `npx --yes md2idx` への参照が残っていないことを機械的に確認し、CI に組み込む
- README / README_ja: Prerequisites を「Node.js 24+ と各バックエンド CLI（shim 実行に POSIX シェル、follow-up 検証に git）」へ更新し、jq / md2idx / npx の記述を削除。How it works の該当箇所を更新
- spec.md / development.md: アーキテクチャ図・shared 同期パターン・モデル追加手順を TS 構成に書き換え
- `jq` / グローバル `md2idx` を PATH から隠した環境で、`gh skill install` 直後の状態から全 skill の実行確認（smoke）
- SKILL.md の呼び出し記述（bash shim 維持 or node 直呼び）を再評価して確定（§5-d）
...
| 候補                                       | 採用 | 理由                                                                                                                                |
| ------------------------------------------ | ---- | ----------------------------------------------------------------------------------------------------------------------------------- |
| **esbuild で単一ファイル JS にバンドル**   | ✓    | ユーザー要件が「Node.js のみ」になる。md2idx / 将来の依存を内包できる。最小 Node は内包する md2idx の engines に合わせて 24（§3.1） |
...
| 候補        | 採用 | 理由                                                                                                                  |
| ----------- | ---- | --------------------------------------------------------------------------------------------------------------------- |
| **esbuild** | ✓    | 単一ファイル・platform=node・決定的出力の CLI バンドルが最小設定で得られる。ビルド高速で pre-commit / CI 検証に耐える |
| `vp pack`   | ✗    | ライブラリ配布（exports / d.ts）向けで、単一ファイル CLI の出力制御が間接的。vp が同等機能を一級サポートしたら再評価  |
| rollup 単体 | ✗    | esbuild と得られる結果が同等で設定量が多い                                                                            |
...
| 候補                                         | 採用 | 理由                                                                                                                                                                                             |
| -------------------------------------------- | ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **mkdir lock に統一 + stale 仕様の新規設計** | ✓    | Node / bash 双方で同一実装が可能で、混在期に相互排他が成立する。現行フォールバックは無期限待ち・stale 対応なしのため、owner 記録 / bounded wait / takeover / trap cleanup を Step 2 で仕様化する |
| flock 優先を維持                             | ✗    | Node 標準に flock がなく、bash=flock / TS=mkdir の混在は相互排他にならない（observe JSON 破壊リスク）                                                                                            |

### g. md2idx の取り込み方

| 候補                             | 採用 | 理由                                                                                                                                                                                                                              |
| -------------------------------- | ---- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **npm 依存として import + 内包** | ✓    | zero-dependency ESM で library exports と型定義を持つ。ただし現行 v0.2.0 は CLI 自己判定が library entry の top-level にあり、バンドル内で誤発火するため、**副作用のない entry を分離した md2idx リリースを先行させる**（Step 1） |
...
- `shared/src/**/*.ts` の in-source test（モジュールごと）
  ...
  - バンドル起動: 最小サポート Node での起動 / md2idx CLI 側コードが発火しない（stdout・exit code 汚染なし）
...
metrics baseline は duration を含まないため、レイテンシ回帰は別途測る。`scripts/run-metrics-fixtures.sh` をベースに、warm-up 1 回 + 本計測 10 回で `prepare` / `read_response` / `dispatch` の `duration_ms` p50 を取り、移行前の記録値との比較で**許容回帰 +20% 以内**を Step 5 / Step 6 / 最終受け入れの完了条件にする。
...
- golden test（wrapper-session / delegate-run / delegate-mcp / metrics baseline）が移行前と同一内容で通る
...
| コミット済み dist と src のドリフト                                         | 決定的ビルド（esbuild バージョンは package-lock 厳守）+ pre-commit / `--check` に加え、pinned 環境の CI で clean checkout から build byte 比較を PR 必須化（Step 1）      |
...
- [md2idx](https://github.com/oubakiou/md2idx) — v0.2.0（zero-dependency ESM / library exports。engines `>=24.0.0`、CLI 自己判定の分離リリースが Step 1 の前提）
- [esbuild](https://esbuild.github.io/) — bundle / platform=node / format=esm
