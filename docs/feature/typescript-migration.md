...
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- | -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [MUST] `shared/` 全スクリプトの TS 化（resolve-model / check-delegate-chain / build·read-request / build·read-response / prepare / dispatch / run / observe-json / backend wrapper 4 本 / prompt-constraints / delegate-mcp） | 未（全て bash、`shared/*.sh` 17 本）                                              | 各スクリプトの argv / stdout / exit code 契約を変えずに TS 実装へ置換。`.sh` は**直接実行されるエントリポイント**の exec shim のみ残る       |          | 進行中（完了: resolve-model / check-delegate-chain / build·read-request / build·read-response / prepare / dispatch / run の 9 本は shim 化済み。observe-json は TS 正本完成・bash 版凍結並存。残: wrapper 4 本 / prompt-constraints / delegate-mcp） |
| [MUST] 専用 wrapper / 専用 run の TS 化（`prepare-imagegen` / `run-imagegen` / `delegate-imagegen-codex` / `run-x-research` / `delegate-x-research-grok`）                                                                    | 未（skill 配下の独立正本 bash）                                                   | 共通 TS モジュールを import する形で置換し、コピー由来重複を解消                                                                             |          | 進行中（Step 5 で `prepare-imagegen` / `run-imagegen` / `run-x-research` を shim 化済み。残: `delegate-imagegen-codex` / `delegate-x-research-grok`（Step 7））                                                                                      |
| [MUST] `jq` 依存の撤廃                                                                                                                                                                                                        | runtime で jq 必須（`shared/` 内 135 箇所超 + 全 SKILL.md の手順・allowed-tools） | 配布物の実行パスと**全 SKILL.md の手順・allowed-tools** に jq への参照が存在しない（静的検査で確認）。README の Prerequisites から jq を削除 |          | 進行中（request/response・resolve/chain・prepare/dispatch/run・専用 run 経路から撤廃済み。observe/wrapper の bash 側と SKILL.md に残存）                                                                                                             |
