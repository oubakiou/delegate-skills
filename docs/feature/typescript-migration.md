...
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- | -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [MUST] `shared/` 全スクリプトの TS 化（resolve-model / check-delegate-chain / build·read-request / build·read-response / prepare / dispatch / run / observe-json / backend wrapper 4 本 / prompt-constraints / delegate-mcp） | 未（全て bash、`shared/*.sh` 17 本）                                              | 各スクリプトの argv / stdout / exit code 契約を変えずに TS 実装へ置換。`.sh` は**直接実行されるエントリポイント**の exec shim のみ残る       |          | 進行中（完了: resolve-model / check-delegate-chain / build·read-request / build·read-response の 6 本は shim 化済み。observe-json は TS 正本完成・bash 版凍結並存。残: prepare / dispatch / run / wrapper 4 本 / prompt-constraints / delegate-mcp） |
...
| [MUST] `jq` 依存の撤廃                                                                                                                                                                                                        | runtime で jq 必須（`shared/` 内 135 箇所超 + 全 SKILL.md の手順・allowed-tools） | 配布物の実行パスと**全 SKILL.md の手順・allowed-tools** に jq への参照が存在しない（静的検査で確認）。README の Prerequisites から jq を削除 |          | 進行中（request/response・resolve/chain 経路から撤廃済み。observe/prepare/dispatch/wrapper の bash 側と SKILL.md に残存）                                                                                                                            |
| [MUST] `md2idx` の runtime `npx` 撤廃                                                                                                                                                                                         | 全 skill が `npx --yes md2idx` に依存                                             | md2idx を npm 依存として import しバンドルに内包。`check-md2idx.sh` 相当の前提チェックが不要になる                                           |          | 進行中（Step 3b で実行パスから npx 撤廃。check-md2idx.sh の削除と SKILL.md 追従は Step 8）                                                                                                                                                           |
...
| [SHOULD] テストの in-source 移行                                                                                                                                                                                              | TS テスト 2 本が bash snippet fixture で約 3,900 行                               | pure ロジックは実装隣接の in-source test へ。fake CLI による wrapper 統合テストは golden として維持                                          |          | 進行中（新規 TS モジュールは in-source + parity で検証。`observe-json.test.ts` の bash snippet describe は bash 版凍結解除（Step 8）まで残す）                                                                                                       |
...
- **着手前に §6 反復ベンチの「移行前」ベースラインを採取して記録する**（bash 版 prepare / dispatch / run が runtime である現 HEAD のうちに。warm-up 1 回 + 本計測 10 回で `prepare` / `read_response` / `dispatch` の `duration_ms` p50）
...
- Step 4b から繰り越した子プロセス密結合の observe 関数もここで移植する: `delegate_observe_wait_with_heartbeat` / `wait_probe_progress` / `process_tree_json` / `codex_home_prune`、および report/prompt 系 helper（`report_mode_for_backend` / `report_schema_json` / `request_inline_*` / `build_response_from_structured` / `assemble_response` / `build_response_from_report_md`）
...
- shim 以外の .sh 実装本体・`check-md2idx.sh`・凍結版 `observe-json.sh`・bash snippet fixture 残骸を削除（bash 版と対で維持してきた `scripts/observe-parity.test.ts` / `scripts/observe-store-parity.test.ts` も同時に削除）
