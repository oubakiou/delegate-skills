...
| [MUST] `jq` 依存の撤廃                                                                                                                                                                                                        | runtime で jq 必須（`shared/` 内 135 箇所超 + 全 SKILL.md の手順・allowed-tools） | 配布物の実行パスと**全 SKILL.md の手順・allowed-tools** に jq への参照が存在しない（静的検査で確認）。README の Prerequisites から jq を削除 |          | 進行中（全実行経路から撤廃済み。残存は未使用の凍結 bash（observe-json / prompt-constraints / delegate-mcp、Step 8 で削除）と SKILL.md の手順・allowed-tools のみ）                                                                                                                                                                      |
