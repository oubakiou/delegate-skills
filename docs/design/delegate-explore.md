...
コードベース・ドキュメント・Web・MCP 経由の外部ナレッジの read-only な探索・読解を、最安モデルの subagent に委譲する skill の設計。
...
- WebSearch / WebFetch による Web 上の調査（ライブラリ仕様・OSS・技術情報）と、実行環境に設定済みの MCP ツール（Notion・Atlassian 等）経由の社内ドキュメント・チケット調査も対象にする
...
- **Web / MCP の開放**: Claude パスは repo 書き込みツールだけを `--disallowedTools` で除外する denylist 方式にし、WebSearch / WebFetch / MCP ツールを技術的に開放する（MCP ツール名は実行環境の MCP 設定依存で allowlist に事前列挙できない）。他パスはプロンプト制約のみで、Web / MCP の可用性は各 CLI の能力次第。MCP 利用は既定で無制限、`DELEGATE_EXPLORE_MCP_READ_ONLY=1` で読み取り系のみに制限するプロンプト制約を注入する（`shared/prompt-constraints.sh`）
...
explore はコード調査・ドキュメント読解・Web 調査・MCP 経由の外部ナレッジ調査を同じ read-only 枠で扱う。実装前に呼び出し関係を追う作業、仕様書から該当要件を拾う作業、ライブラリの最新仕様を Web で確認する作業、Notion / Jira から関連ドキュメント・チケットを拾う作業は、いずれもリポジトリへの副作用がなく、安価な探索としてまとめて委譲できる。副作用がないため失敗時の巻き戻しも不要で、低リスクな前処理として使いやすい。

Web / MCP 由来の untrusted コンテンツは子プロセスに隔離され、main には報告 Markdown だけが返る（context isolation が prompt injection の影響範囲も限定する）。worker には取得コンテンツ内の指示に従わない制約を固定テンプレで伝える。
