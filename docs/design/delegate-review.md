...
review の作業を委譲する場合は、この skill の固定フローを使う。generic な subagent へ流す運用は想定しない。
...
- **制約**: ファイル編集・git の書き込み操作・push はしない。実行系の sandbox 設定はプロトコル共通（spec.md [§5](spec.md#5-実行系の五分岐)）
