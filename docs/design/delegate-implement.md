...
implement の作業を委譲する場合は、この skill の固定フローを使う。generic な subagent へ流す運用は想定しない。
...
既定は通常 run のままにする。複数ファイル実装や大きめの修正など、親の差分確認で差し戻す可能性が高い場合だけ、親は初回 request から `prepare.sh` 第 6 引数に `resumable` を渡して resumable initial run を明示する。response 読了後は observe JSON の `backend_session.persistence` を確認し、`resumable` 以外なら follow-up 不可としてその時点で判断する。

親チェックで不具合が見つかった場合は、前回 observe JSON を指定して `prepare.sh` 第 6 引数に `followup=<前回observe_file>` を渡す。prepare が exit 5 を返した場合は resume 不可または stale-context 検出なので中止し、暗黙に新規 run へ落とさない。follow-up request には、親が見つけた不具合、最新 `git diff` の見るべき範囲、前回 `response_file` / Verification の参照を含め、worker の古い会話文脈だけに依存させない。
...
- 多段委譲ポリシー（`implement` がチェーンに二度登場するのを禁止）: spec.md [§8](spec.md#8-多段委譲ポリシー再帰防止)
