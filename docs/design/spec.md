...
| `delegate-htmldoc`                              | 固定テンプレートによる HTML ドキュメント生成       | 出力ファイル書き込みのみ（push なし）            | `haiku`      | `DELEGATE_HTMLDOC_MODEL` / `DELEGATE_WORK_DIR`                                   |
...
- htmldoc は skill 同梱の固定テンプレート（CSS + component 語彙）へ content を流し込むだけで判断比重が低いため `haiku`。デザインの一貫性はモデルではなくテンプレート資産で担保する
