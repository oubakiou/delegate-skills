...
| `delegate-imagegen`                           | 画像生成/編集の capability bridge          | Codex 子プロセス             | Codex 既定 | `DELEGATE_WORK_DIR` / `DELEGATE_IMAGEGEN_OUTPUT_DIR` |
...
- imagegen は既存の `DELEGATE_<TYPE>_MODEL` / `gpt-*` モデル解決とは混ぜず、Codex 限定の実行系として扱う。主目的は token cost 削減ではなく capability bridge と context isolation で、ユーザー向けには画像生成モデル選択の概念を持たせない。出力先は明示がなければ `.temp/imagegen/` 配下
