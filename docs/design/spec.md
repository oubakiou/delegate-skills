...
| [`delegate-x-research`](delegate-x-research.md) | x.com / X 調査の capability bridge         | X 調査子プロセス             | `grok-4.3` | `DELEGATE_X_RESEARCH_MODEL` / `DELEGATE_WORK_DIR`                                |
...
- xresearch は `DELEGATE_X_RESEARCH_MODEL` → 既定 `grok-4.3` で解決し、X 調査 capability bridge として扱う。現在の実装 backend は Grok CLI。X の投稿・検索結果は時点依存なので、worker report に確認時刻と根拠 URL を残す
...
| `DELEGATE_X_RESEARCH_MODEL`    | `grok-4.3`                               | `delegate-x-research` の X 調査 backend に渡すモデル             |
