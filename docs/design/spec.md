...
| `build-request.sh`        | リクエスト生成（命名・md2idx・envelope 付与） |
| `read-request.sh`         | リクエストの段階読み取り（worker 側）         |
| `build-response.sh`       | レスポンス生成（worker 側）                   |
| `read-response.sh`        | レスポンスの段階読み取り（main 側）           |
