...
| [`delegate-review`](delegate-review.md)       | コードレビュー（差分の指摘）               | read-only（Read/Grep/Glob）  | `opus`     | `DELEGATE_REVIEW_MODEL` / `DELEGATE_WORK_DIR`    |
...
- `delegate-imagegen`: Claude Code など画像生成手段を持たない requester から、画像生成 capability を持つ Codex 子プロセスを呼ぶ専用 skill を検討する。主目的は token cost 削減ではなく capability bridge と context isolation とし、プロンプト試行錯誤・生成パラメータ・失敗ログ・成果物検証を worker 側に閉じ込め、main は生成ファイル一覧と短い報告だけを読む。既存の `DELEGATE_<TYPE>_MODEL` / `gpt-*` モデル解決とは混ぜず、Codex 限定の実行系として扱い、ユーザー向けには画像生成モデル選択の概念を持たせない。出力先は明示がなければ `.temp/imagegen/` 配下、response section は `Summary` / `Generated files` / `Parameters` / `Verification` / `Blockers` を基本にする
- `session resume`: `responder_session_id` を手掛かりに、前回の実装時コンテキストへ戻って修正を続行できる運用を検討する。Claude は `claude --resume <session-id>` / `/resume` で同一 project の session history を再開できる場合に限って再利用し、Codex は conversation / thread state を継続できる surface / API がある場合に限って再利用する。どちらも再開できない場合は新しい `delegate-implement` request を発行し、前回の request / response / diff を再投入してコンテキストを再構成する。この protocol 自体には再接続の標準化は含めない
