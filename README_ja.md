...
| 実行系           | モデル名                                                                                                                                                                                                                                                                                                                                                                        | 補足                                                                                                                                                                                                       |
| ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Claude CLI       | `fable`, `opus`, `sonnet`, `haiku`, `claude-fable-5-1`, `claude-fable-5`, `claude-opus-5-5`, `claude-opus-5`, `claude-opus-5[1m]`, `claude-opus-4-8`, `claude-opus-4-8[1m]`, `claude-sonnet-5`, `claude-sonnet-5[1m]`, `claude-sonnet-4-6`, `claude-haiku-4-5`                                                                                                                  | Claude 系モデルの alias に加え、バージョン固定用にフル `claude-*` モデル ID も指定可（`[1m]` は 1M context 変種）                                                                                          |
| Codex CLI        | `gpt-6-astra`, `gpt-6-sol`, `gpt-6-luna`, `gpt-5.6`, `gpt-5.6-sol`, `gpt-5.6-terra`, `gpt-5.6-luna`, `gpt-5`, `gpt-5.5`, `gpt-5.4`, `gpt-5.4-mini`, `gpt-5.4-nano`, `gpt-5.3-codex-spark`                                                                                                                                                                                       | `delegate-imagegen` は `gpt*` / Codex 分岐のみ受け付ける                                                                                                                                                   |
| Devin CLI        | `swe-2`, `swe-1.7`, `swe-1.7-lightning`, `swe-1.6`, `swe-1.6-fast`, `devin-glm-5.2`, `devin-deepseek-v4-pro`, `devin-nemotron-3-ultra`, `devin-kimi-k3`, `devin-gemini-3.8-flash`                                                                                                                                                                                               | `devin-*` は prefix を剥がして Devin CLI に渡す                                                                                                                                                            |
...
| opencode CLI     | `opencode/opencode-go/glm-5.2`, `opencode/opencode/muse-spark-1.3-contributor-free`                                                                                                                                                                                                                                                                                             | 記法は `opencode/<provider>/<model>`（provider の省略不可）。実機往復を確認したモデル。他は `opencode models` の出力から選ぶ。`muse-spark-1.3-contributor-free` は価格とデータを交換するモデル（下記参照） |
...
opencode の free / contributor モデルは価格とデータを交換する。opencode のドキュメントは、contributor 枠を「プロンプトと生成結果を将来のモデル学習に使う許可と引き換えに、トークン単価を大幅に割り引くもの」と説明し、いくつかの free モデルについても収集データをモデル改善に使い得ると記載する。有料モデルは zero-retention とされている。委譲 run はリポジトリの内容を子プロセスへ渡すため、`opencode/opencode/muse-spark-1.3-contributor-free` と価格表の他の無料 opencode 行は、共有できないコードには使わない。条件は変わり得るので、委譲前に opencode 側のドキュメントで最新の条件を確認する。
...
| `cursor-grok-4.7` / `cursor-grok-4.7-fast` | `low`, `medium`, `high`, `xhigh`                  | `grok-4.7-<effort>` / `grok-4.7-<effort>-fast` へ変換（4.7 の catalog slug には `cursor-` prefix が付かない） |
...
`cursor-` selector を使う Cursor モデルでは、先頭 `cursor-` は delegate の backend selector であり、ちょうど 1 回だけ付ける。二重 prefix（`cursor-cursor-*`）や grok の effort catalog slug の直指定（`cursor-grok-4.5-low` / `-medium` / `-high`、`cursor-grok-4.6-low` / `-medium` / `-high` / `-xhigh`、`cursor-grok-4.7-low` / `-medium` / `-high` / `-xhigh`、および対応する `-fast` 付き）は dispatch 前に exit 6 で停止する。代わりに `cursor-grok-4.5[@<effort>]`、`cursor-grok-4.6[-fast][@<effort>]`、`cursor-grok-4.7[-fast][@<effort>]` の表記を使うこと。既存 session がこれらの旧表記を保持している場合、その follow-up は再開できず（exit 5）、修正した表記で新規 resumable run を開始する。

suffix を付けない場合、delegate-skills は effort を明示せず、実行先 CLI の既定値を使う。例外は、catalog 既定が `medium` の `gpt-5.5` / `gpt-5.4` / `gpt-5.4-mini` と `cursor-grok-4.5`、effort が `high` に解決される `cursor-grok-4.6` / `cursor-grok-4.6-fast` / `cursor-grok-4.7` / `cursor-grok-4.7-fast`、および model slug 自体に effort を含む Cursor モデル（`-high` / `-max`、および `cursor-gemini-3.6-flash-*` 変種）である。Devin では `devin-gemini-3.8-flash` と `swe-2` は suffix を省略すると catalog 既定 variant の `medium` へ解決される（実 CLI で確認済み。他の Devin family は未確認）。

`cursor-grok-4.6` は effort を省略すると `cursor-grok-4.6-high` へ変換されるため、non-fast のままになる。catalog の bare `grok-4.6` は `high` かつ `fast` を既定にするため、これをそのまま渡さない。fast は `cursor-grok-4.6-fast` を明示した場合だけ選ばれ、effort 省略時は `cursor-grok-4.6-high-fast` へ変換される。

`cursor-grok-4.7` も同様だが、4.7 の catalog slug には `cursor-` prefix が付かない。effort を省略すると non-fast の `grok-4.7-high` へ変換され、`cursor-grok-4.7-fast` は effort 省略時に `grok-4.7-high-fast` へ変換される。
...
`gpt-6-astra` と `gpt-6-sol` も上記の Codex effort suffix（`low`〜`ultra`）に対応する。Codex のローカルモデル一覧で確認済み。`gpt-6-luna` と `gpt-5.6-luna` が公開する effort は `low`〜`max` のみである。delegate-skills は effort をモデル単位ではなく backend 単位で検証するため、これらに `@ultra` を付けても検証は通り、受理するかどうかは Codex CLI 側の判断になる。catalog 既定値は `gpt-6-astra` と `gpt-5.6-sol` が `low`、`gpt-6-sol` と `gpt-6-luna` が `medium` だが、suffix を省略した場合の実効値は実行先 CLI の設定と既定値に依存する。
