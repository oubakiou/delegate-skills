...
`delegate-explore` はコード・リポジトリ内ドキュメントに加え、WebSearch / WebFetch による Web 調査と、実行環境に設定済みの MCP ツール（Notion・Atlassian 等）経由の社内ナレッジ調査も対象にする。Claude backend ではこれらのツールを技術的に開放する（リポジトリ書き込みツールのみ denylist で除外）。他 backend ではプロンプト制約での伝達となり可用性は各 CLI の能力次第のため、Web / MCP 調査には Claude 系モデルを推奨する。worker の MCP 利用は常に読み取り系ツールのみに制限される（プロンプトレベルの制約）。MCP への書き込みを伴う作業は `delegate-chore` / `delegate-implement` に委譲する。取得した Web / MCP コンテンツ（prompt injection リスクを含む）は子プロセスに隔離され、main agent には worker の報告だけが返る。
