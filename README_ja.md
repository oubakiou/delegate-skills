...
**実装・調査・git 操作・レビュー・雑務などのタスクを、安価なモデルの subagent に委譲してトークン費用を圧縮する LLM エージェント向け skill 集。**
...
| `delegate-git`       | git + gh 操作（push/PR 可）                | general-purpose | git/gh 限定はプロンプト制約で担保 | `haiku`    | `DELEGATE_GIT_MODEL` / `DELEGATE_WORK_DIR`       |
...
既定モデルの根拠: explore / chore は read 中心・低リスクで `haiku`、git は取り消し困難な操作を含むが判断をサブエージェントに委ねず明確な指示で単純操作のみ delegate するため `haiku`、implement は編集の判断を要するため `sonnet`、review は指摘品質が成果物に直結し判断比重が高いため `opus`。
