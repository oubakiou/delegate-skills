...
| Devin CLI        | `swe-1.7`, `swe-1.7-lightning`, `swe-1.6`, `swe-1.6-fast`, `devin-glm-5.2`, `devin-deepseek-v4-pro`, `devin-nemotron-3-ultra`, `devin-kimi-k3`, `devin-gemini-3.8-flash`                                                                                                                                                             | `devin-*` は prefix を剥がして Devin CLI に渡す                                                                                                                                                            |
...
suffix を付けない場合、delegate-skills は effort を明示せず、実行先 CLI の既定値を使う。例外は、catalog 既定が `medium` の `gpt-5.5` / `gpt-5.4` / `gpt-5.4-mini` と `cursor-grok-4.5`、effort が `high` に解決される `cursor-grok-4.6` / `cursor-grok-4.6-fast`、および model slug 自体に effort を含む Cursor モデル（`-high` / `-max`、および `cursor-gemini-3.6-flash-*` 変種）である。Devin では `devin-gemini-3.8-flash` は suffix を省略すると catalog 既定 variant の `medium` へ解決される（実 CLI で確認済み。他の Devin family は未確認）。
