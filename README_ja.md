...
`delegate-explore` はコード・リポジトリ内ドキュメントに加え、WebSearch / WebFetch による Web 調査と、実行環境に設定済みの MCP ツール（Notion・Atlassian 等）経由の社内ナレッジ調査も対象にする。Claude backend ではこれらのツールを技術的に開放する（リポジトリ書き込みツールのみ denylist で除外）。他 backend ではプロンプト制約での伝達となり可用性は各 CLI の能力次第のため、Web / MCP 調査には Claude 系モデルを推奨する。MCP 利用は既定で無制限、`DELEGATE_EXPLORE_MCP_READ_ONLY=1` で読み取り系 MCP ツールのみに制限するプロンプトレベルの制約を注入する。取得した Web / MCP コンテンツ（prompt injection リスクを含む）は子プロセスに隔離され、main agent には worker の報告だけが返る。
...
| --------------------- | -------------------------------------------------- | ---------------------------- | ------------ | --------------------------------------------------------------------------------- |
| `delegate-explore`    | read-only のコード/ドキュメント/Web/MCP 探索・読解 | read-only（Web・MCP 可）     | `haiku`      | `DELEGATE_EXPLORE_MODEL` / `DELEGATE_EXPLORE_MCP_READ_ONLY` / `DELEGATE_WORK_DIR` |
...
| `DELEGATE_EXPLORE_MCP_READ_ONLY`         | `0`（無制限）                            | `1` で explore worker の MCP 利用を読み取り系ツールのみに制限（プロンプトレベルの制約） |
